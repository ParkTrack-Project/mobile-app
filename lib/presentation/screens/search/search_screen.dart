...
class _SearchScreenState extends ConsumerState<SearchScreen> {
  static const BoundingBox _globalBoundingBox = BoundingBox(
    southWest: Point(latitude: -85.0, longitude: -180.0),
    northEast: Point(latitude: 85.0, longitude: 180.0),
  );
  ...
  Future<void> _suggest(String text) async {
    ...
    try {
      ...
      if (items.isEmpty && searchBias != null) {
        items = await _loadSuggestions(text, _globalBoundingBox);
      }
      if (!mounted) return;
      setState(() {
        _suggestions = items;
  ...
  Future<void> _onSuggestionTap(SuggestItem item) async {
    ...
    try {
      ...
      final preferredBoundingBox = searchBias == null
          ? _globalBoundingBox
          : _buildLocalBoundingBox(searchBias);
      var point = await _searchPointByText(
        item.searchText,
        preferredBoundingBox,
      );
      if (point == null && searchBias != null) {
        point = await _searchPointByText(item.searchText, _globalBoundingBox);
      }
      if (!mounted) return;
      if (point != null) {
        ref.read(destinationProvider.notifier).state = Destination(
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Не удалось определить координаты места'),
          ),
        );
      }
    } catch (_) {
