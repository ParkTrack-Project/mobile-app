...
{
  private static final int MAX_ICON_DIMENSION_PX = 1024;
  ...
  private ImageProvider getIconImage(Map<String, Object> image) {
    ...
    if (type.equals("fromAssetImage")) {
      ...
      try (InputStream i = controller.get().context.getAssets().open(assetName)) {
        Bitmap result = decodeSampledBitmap(readBytes(i));

        return result != null ? ImageProvider.fromBitmap(result) : defaultImage;
      } catch (IOException e) {
        return defaultImage;
      }
    }
    ...
    if (type.equals("fromBytes")) {
      ...
      Bitmap bitmap = decodeSampledBitmap(rawImageData);
  ...
  private static Bitmap decodeSampledBitmap(byte[] encodedImage) {
    ...
    return BitmapFactory.decodeByteArray(encodedImage, 0, encodedImage.length, options);
  }
  ...
  private static int calculateInSampleSize(int width, int height) {
    ...
    while (
      width / sampleSize > MAX_ICON_DIMENSION_PX ||
      height / sampleSize > MAX_ICON_DIMENSION_PX
    ) {
      sampleSize *= 2;
    }
